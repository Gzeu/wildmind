...
import { getHighScore } from '../services/HighScore';
...
export class MenuScene extends Container {
  ...
  private build(): void {
    ...
    if (hs > 0) {
      const hsStyle = new TextStyle({ fill: '#f5c842', fontSize: 15, fontFamily: 'monospace' });
      const hsTxt = new Text({ text: `🏆 Best: ${hs} pts`, style: hsStyle });
    ...
    const footer = new Text({ text: 'WildMind v1.1 · 5 rounds · Guess the animal before the blur clears', style: footerStyle });
