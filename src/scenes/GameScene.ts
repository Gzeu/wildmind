...
export class GameScene extends Container {
  ...
  constructor(game: Game) {
    super();
    this.game = game;
    // Kick off async build; guard with this.destroyed checks inside
    void this.build();
  }
  ...
  private async build(): Promise<void> {
    ...
    // Mute toggle button
    const muteStyle = new TextStyle({ fontSize: 20 });
    const muteTxt = new Text({ text: '🔊', style: muteStyle });
    muteTxt.x = W - 44;
    muteTxt.y = 12;
    ...
    imgBg.roundRect(imgX, imgY, imgSize, imgSize, 16).fill({ color: 0x1a5c36, alpha: 0.4 });
    this.addChild(imgBg);
    ...
    try {
      ...
      this.ticker = (ticker) => {
        if (!this.guessed) {
          ...
          this.updateTimer(imgX, imgY, imgSize);
        }
      };
      ...
      // Auto-advance after 2s on image error
      setTimeout(() => {
        if (!this.destroyed) {
    ...
    timerBg.roundRect(imgX, imgY + imgSize + 10, imgSize, 8, 4).fill({ color: 0x1a5c36 });
  ...
  private updateTimer(imgX: number, imgY: number, imgSize: number): void {
    if (!this.blurReveal) return;
    const progress = this.blurReveal.progress;
    const secsLeft = Math.ceil(9 * (1 - progress));

    // Tick sound on each new second in the last 3 seconds
    if (secsLeft <= 3 && secsLeft !== this.lastTickSec) {
  ...
  private handleGuess(choice: string, btn: Button, correctName: string): void {
    ...
    const H = this.game.app.screen.height;
    const fact = new Text({
      text: `Fun fact: ${this.game.currentAnimal.funFact}`,
      style: new TextStyle({
